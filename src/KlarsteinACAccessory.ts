...
export class KlarsteinACAccessory implements AccessoryPlugin {
  ...
  private readonly hap: typeof import('hap-nodejs');
  ...
  private createSignature(httpMethod: string, url: string, body: string = ''): { sign: string; t: string; nonce: string } {
  ...
  private createBusinessSignature(httpMethod: string, url: string, body: string = ''): { sign: string; t: string; nonce: string } {
    const t = Date.now().toString();
    const nonce = this.generateUUID().replace(/-/g, '');
  ...
  private async setStatus(code: string, value: any): Promise<void> {
    ...
    try {
      await axios.post(`${this.apiBase}${url}`, {
        commands: [{ code, value }],
      }, {
        headers: {
          'client_id': this.clientId,
